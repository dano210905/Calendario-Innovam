...
public class AuthService
{
    ...
    public string? AvatarUrl { get; set; } // URL del avatar/foto de perfil del usuario
    ...
    public async Task<(bool ok, string msg)> Register(
        string nombre,
        string email,
        string contrasena,
        int idRol // Nuevo parámetro que viene del <select>
    )
    {
        try
        {
            // 1. Preparamos los datos del usuario
            // Nota: He mantenido las llaves tal cual las tenías para que coincidan con tu API
            ...
            var (userOk, userMsg) = await PostJson(
                "usuario?camposEncriptar=contrasena",
                datosUsuario
            );

            if (!userOk)
                return (false, $"Error al crear usuario: {userMsg}");

            // 3. Si el usuario se creó bien, le asignamos el rol
            // Usamos el idRol que recibimos de la vista (2 o 3)
            var datosRol = new Dictionary<string, object?>
            {
                ["fkemail"] = email, // Se vincula por el correo según tu tabla dbo.rol_usuario
                ["fkidrol"] = idRol,
            };

            var (rolOk, rolMsg) = await PostJson("rol_usuario", datosRol);

            if (!rolOk)
            {
                // Opcional: Podrías loguear que el usuario se creó pero no el rol
                return (false, $"Usuario creado, pero hubo un error con el rol: {rolMsg}");
            }
    ...
    )
    {
        try
        {
            ...
            if (
                doc.RootElement.TryGetProperty("resultados", out var resultados)
    ...
    private static bool _estructuraCargada = false;
    ...
    {
        if (_estructuraCargada)
            return;
        try
        {
            var json = await _http.GetStringAsync("/api/estructuras/basedatos");
            using var doc = JsonDocument.Parse(json);
            if (!doc.RootElement.TryGetProperty("tablas", out var tablas))
                return;
            ...
            foreach (var t in tablas.EnumerateArray())
            {
                ...
                var nombre =
                    (
                        t.TryGetProperty("table_name", out var tn) ? tn.GetString()
                ...
                if (t.TryGetProperty("columnas", out var colArr))
                    foreach (var col in colArr.EnumerateArray())
                    {
                        ...
                        if (
                            !dict.ContainsKey("is_primary_key")
                            && dict.ContainsKey("es_primary_key")
                        )
                            dict["is_primary_key"] =
                                dict["es_primary_key"]?.ToString() == "True" ? "YES" : "NO";
                        columnas.Add(dict);
                    }
                ...
                foreach (var col in columnas)
                    ...
                    {
                        _cache[$"pk_{nombre}"] = col["column_name"]!.ToString()!;
                        break;
                    }
    ...
    public async Task<(bool ok, string msg)> Login(string email, string contrasena)
    {
        try
        {
            // 1. Disparar la precarga en segundo plano (NO LE HACEMOS AWAIT AQUÍ)
            // Se ejecuta en un hilo separado mientras el usuario se autentica
            _ = PrecargarEstructura();

            // 2. Autenticar inmediatamente con la API (Esto solo tarda 611 ms)
            var authTask = PostJson(
                "autenticacion/token",
            ...
            // Esperamos ÚNICAMENTE a la validación de la contraseña
            var (ok, msg) = await authTask;
            if (!ok)
            ...
            if (Roles.Count == 0)
            {
                return (false, "¡Contraseña OK, pero NO TIENES ROLES!");
            }

            // ── PASO 3: ENCRIPTACION Y SESION ──
            ...
            // Guardar avatar si ya fue seleccionado (se actualiza desde Registro o perfil)
            if (!string.IsNullOrEmpty(AvatarUrl))
                await _session.SetAsync("avatar_url", AvatarUrl);
...
WHERE u.{pkUsuario} = @email";
            ...
            var resultados = await PostConsulta(
                sql,
            ...
            foreach (var u in usuarios)
            {
                ...
                if (val.Equals(email, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    DebeCambiarContrasena =
                        debeCambiar == "True" || debeCambiar == "true" || debeCambiar == "1";
                    break;
                }
            }
        }
    ...
    private async Task CargarRutasPermitidasFallback()
    {
        ...
        try
        {
            ...
            foreach (var rr in rutasRol)
            {
                ...
                if (string.IsNullOrEmpty(ruta))
                    ruta =
                        rr.GetValueOrDefault("fkruta")?.ToString()
        ...
        try
        {
            ...
            if (userResult.Success && !string.IsNullOrEmpty(userResult.Value))
            {
                ...
                if (nombreResult.Success)
                    NombreUsuario = nombreResult.Value;
                ...
                // Restaurar avatar/foto de perfil
                var avatarResult = await _session.GetAsync<string>("avatar_url");
                if (avatarResult.Success && !string.IsNullOrEmpty(avatarResult.Value))
    ...
    public async Task Logout()
    {
        ...
        await _session.DeleteAsync("avatar_url");
        AvatarUrl = null;
    }
    ...
    /// </summary>
    public async Task<(bool ok, string msg)> RecuperarContrasena(
        string email,
        ...
        try
        {
            ...
            var pwd = new string(
                new[]
                {
                    ...
                    .Concat(Enumerable.Range(0, 5).Select(_ => all[rng.Next(all.Length)]))
                    .ToArray()
            );
            ...
            catch (Exception ex)
            {
                return (
                    true,
                    $"Contrasena restablecida pero no se pudo enviar el correo: {ex.Message}"
                );
            }
        }
        catch (Exception ex)
    ...
    private async Task<(bool ok, string msg)> CambiarContrasenaInterno(string email, string nueva)
    {
        ...
        var resp = await _http.PutAsync(
            $"/api/usuario/{pkUsuario}/{email}?camposEncriptar=contrasena",
            content
        );
        if (resp.IsSuccessStatusCode)
            return (true, "OK");
